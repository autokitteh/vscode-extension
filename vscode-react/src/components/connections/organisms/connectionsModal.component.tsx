...
export const ConnectionsModal = ({ onClose }: { onClose: () => void }) => {
	...
	const handleConnectionInitClick = (connectionName: string, connectionInitURL: string, connectionId: string) => {
		sendMessage(MessageType.openConnectionInitURL, {
			connectionId,
			connectionName,
			initURL: connectionInitURL,
		});
	};
					...
					<div className="mt-2 flex w-full justify-end">
						{connections?.length ? (
							<Table>
								...
								{connections.map(({ connectionId, initURL, integrationName, name, status, statusInfoMessage }) => (
									<Row key={connectionId}>
										...
										<Cell classes={["flex justify-center align-center"]}>
											{initURL && (
												<div
													className="codicon codicon-gear w-3 cursor-pointer text-white"
