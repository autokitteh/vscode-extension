import { parseTiltfileError, Location } from "./errorParser";
...
export class TiltfileErrorWatcher implements Disposable {
	...
	private tiltWatch: ChildProcessWithoutNullStreams | undefined;
				...
				try {
					...
					if (result) {
						...
						locations.forEach((location) => {
							...
							this.diagnostics.set(uri, [tiltfileErrorToDiagnostic(message, location)]);
						});
					}
				} catch (error) {
					this.output.appendLine(`tilt session watch: error processing session json: ${error}`);
				}
			}
		});
		p.on("close", (code) => {
...
function tiltfileErrorToDiagnostic(message: string, location: Location): vscode.Diagnostic {
