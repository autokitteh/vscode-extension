...
export const sidebarControllerRefreshRate: number =
	Math.max(
		0,
		getConfig<number>("autokitteh.sidebar.refresh.interval", DEFAULT_SIDEBAR_VIEW_REFRESH_INTERVAL)
	) * 1000;
...
export const projectControllerRefreshRate: number =
	Math.max(
		0,
		getConfig<number>("autokitteh.project.refresh.interval", DEFAULT_PROJECT_VIEW_REFRESH_INTERVAL)
	) * 1000;
