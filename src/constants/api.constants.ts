...
export const baseURLFromVSCode: string = getConfig<string>("baseURL", "");

export const BASE_URL = ValidateURL(baseURLFromVSCode) ? baseURLFromVSCode : "";

export const sidebarControllerRefreshRate: number =
	Math.max(0, getConfig<number>("sidebar.refresh.interval", DEFAULT_SIDEBAR_VIEW_REFRESH_INTERVAL)) * 1000;
...
export const projectControllerRefreshRate: number =
	Math.max(0, getConfig<number>("project.refresh.interval", DEFAULT_PROJECT_VIEW_REFRESH_INTERVAL)) * 1000;
